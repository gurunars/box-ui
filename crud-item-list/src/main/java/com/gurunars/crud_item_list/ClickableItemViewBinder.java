...
class ClickableItemViewBinder<ItemType extends Item> implements ItemViewBinder<SelectableItem<ItemType>> {
    ...
    @Override
    public void bind(View itemView, final SelectableItem<ItemType> item,
                     @Nullable SelectableItem<ItemType> previousItem) {
        itemViewBinder.bind(itemView, item, previousItem);

        itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                collectionManager.itemClick(item.getItem());
            }
        });
        itemView.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                collectionManager.itemLongClick(item.getItem());
                return true;
            }
        });
    }
